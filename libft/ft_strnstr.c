...
char	*ft_strnstr(const char *haystack, const char *needle, size_t len)
{
	...
	if (*needle == '\0')
		return ((char *)haystack); // If needle is an empty string, return haystack
	i = 0;
	while (haystack[i] && i < len)
	{
		j = 0;
		while (haystack[i + j] == needle[j] && (i + j) < len) // Ensure we don't exceed len
		{
			if (needle[j + 1] == '\0')
				return ((char *)(haystack + i)); // Found the entire needle
			j++;
		}
		i++;
	}
